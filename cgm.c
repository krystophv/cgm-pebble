    ...
    if (current_values == NULL) {
        ...
        while (o != NULL) {
            ...
            case 10:
                ...
                rawvibrate = atoi(o);
                if (rawvibrate == 0) {
                    TurnOffVibrationsCalcRaw = 111;
                } else {
                    TurnOffVibrationsCalcRaw = 100;
                }
                break;
            }
            o = strtok(NULL, ",");
...
void fetch_timer_callback(void *data);
...
void bg_vibrator (uint16_t BG_BOTTOM_INDX, uint16_t BG_TOP_INDX, uint8_t BG_SNOOZE, uint8_t *bg_overwrite, uint8_t BG_VIBE) {
    ...
    if ( ( ((conv_vibrator_bg > BG_BOTTOM_INDX) && (conv_vibrator_bg <= BG_TOP_INDX))
        ...
        if ((last_alert_time == 0) || (*bg_overwrite == 100)) {
...
static void load_bg() {
    ...
    // FOR TESTING ONLY
    //strncpy(current_bg_str, "5", BG_BUFFER_SIZE);
    ...
    if ((current_bg <= 0) || (current_bg_str[0] == '-')) {
        ...
        if ( ((current_bg > 0) && (current_bg < bg_ptr[SPECVALUE_BG_INDX])) && (have_calc_raw == 111) ) {
...
void window_load_cgm(Window *window_cgm) {
    ...
    fetch_timer = app_timer_register((LOADING_MSGSEND_SECS * MS_IN_A_SECOND), fetch_timer_callback, NULL);
